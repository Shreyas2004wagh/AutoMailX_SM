...
import { LogOut, Inbox, AlertTriangle, Menu, X, Calendar } from "lucide-react"; // Import Calendar icon
...
interface Email {
  ...
  from: string;
  sender: string;
  //priority: string; // Remove - you're replacing with category
  //sentiment: string; // Remove, unless you add explicit sentiment analysis
  subject: string;
...
function Content() {
 ...
 <nav className={`fixed md:static inset-y-0 left-0 w-64 md:w-48 bg-black/40 backdrop-blur-sm border-r border-purple-500/20 p-4 transition-transform z-30 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
    ...
    {/* All Emails */}
    <button
        className={`flex items-center gap-2 w-full px-4 py-2 rounded-lg transition-colors ${activeFilter === "all" ? "bg-purple-500/30 text-purple-100" : "text-purple-200 hover:bg-purple-500/20"}`}
    ...
    {/* Urgent */}
    <button
        className={`flex items-center gap-2 w-full px-4 py-2 rounded-lg transition-colors ${activeFilter === "urgent" ? "bg-purple-500/30 text-purple-100" : "text-purple-200 hover:bg-purple-500/20"}`}
   ...
   {/* Positive */}
<button
    className={`flex items-center gap-2 w-full px-4 py-2 rounded-lg transition-colors ${activeFilter === "positive" ? "bg-purple-500/30 text-purple-100" : "text-purple-200 hover:bg-purple-500/20"}`}
...
{/* Neutral */}
<button
    className={`flex items-center gap-2 w-full px-4 py-2 rounded-lg transition-colors ${activeFilter === "neutral" ? "bg-purple-500/30 text-purple-100" : "text-purple-200 hover:bg-purple-500/20"}`}
...
>
    ...
    {/* Calendar */}
    <button
        className={`flex items-center gap-2 w-full px-4 py-2 rounded-lg transition-colors ${activeFilter === "calendar" ? "bg-purple-500/30 text-purple-100" : "text-purple-200 hover:bg-purple-500/20"}`}
        ...
        <span>Calendar</span>
                    ...
                    {filteredEmails.length > 0 ? (
                        filteredEmails.map((email) => {
                            ...
                            return (
                                <div
                                    key={email.id}
                                    className={`p-4 border-b border-purple-500/20 cursor-pointer transition-colors ${selectedEmail?.id === email.id ? "bg-purple-500/30" : "hover:bg-purple-500/20"
                                        }`}
                                    onClick={() => {
                                ...
                                >
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                           {email.category === "urgent" && <AlertTriangle className="w-4 h-4 text-red-400" />}
           ...
           />
               ...
               <div className={`flex-1 bg-black/40 backdrop-blur-sm p-4 md:p-6 overflow-y-auto ${isEmailListOpen && "hidden md:block"}`}>
                    {selectedEmail ? (
                        <>
                            <button className="md:hidden flex items-center gap-2 text-purple-200 mb-4" onClick={() => setIsEmailListOpen(true)}>
                                ← Back to list
                            </button>

                 {/* Display AI Summary (optional - you'd need a separate summary generation endpoint) */}
